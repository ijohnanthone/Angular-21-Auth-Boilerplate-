...
function createAccount() {
        ...
        function deleteAccount() {
            ...
            if (idFromUrl() !== currentAccount().id && !isAuthorized(Role.Admin)) {
                return unauthorized();
            }
        ...
        function ok(body?: any) {
            return of(new HttpResponse({ status: 200, body }))
                .pipe(delay(500)); // delay response to simulate network latency
        }
        ...
        function generateRefreshToken() {
            return `fake-refresh-token.${btoa(Math.random().toString())}`;
        }

        function getRefreshToken() {
            return headers.get('Authorization-Refresh-Token');
        }
